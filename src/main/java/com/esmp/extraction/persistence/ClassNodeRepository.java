package com.esmp.extraction.persistence;

import com.esmp.extraction.model.ClassNode;
import org.springframework.data.neo4j.repository.Neo4jRepository;
import org.springframework.data.neo4j.repository.query.Query;
import org.springframework.stereotype.Repository;

/**
 * Spring Data Neo4j repository for {@link ClassNode} entities.
 *
 * <p>Uses the business-key {@code fullyQualifiedName} as the {@code @Id} so that {@code saveAll()}
 * performs idempotent MERGE semantics rather than always creating new nodes.
 */
@Repository
public interface ClassNodeRepository extends Neo4jRepository<ClassNode, String> {

  /** Returns the total number of JavaClass nodes in the graph. */
  @Query("MATCH (c:JavaClass) RETURN count(c)")
  long countAll();
}
